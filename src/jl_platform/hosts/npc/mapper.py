...
class NPCHostAdapter(HostAdapter):
    ...
    def register_tools(self, registry) -> None:
        from jl_platform.hosts.npc.tools import register_npc_tools

        register_npc_tools(registry)

    def postprocess(self, output) -> Dict[str, Any]:
        from jl_platform.hosts.npc.outputs import to_npc_decision

        return to_npc_decision(output).dict()
