...
# If this script is being run inside an activated virtualenv, deactivate it
if ($env:VIRTUAL_ENV) {
    Write-Host "[SYSTEM] Detected active virtualenv at $env:VIRTUAL_ENV. Deactivating..." -ForegroundColor Yellow
    $deactivateScript = Join-Path $env:VIRTUAL_ENV 'Scripts\Deactivate.ps1'
    if (Test-Path $deactivateScript) {
        & $deactivateScript
    } elseif (Get-Command deactivate -ErrorAction SilentlyContinue) {
        deactivate
    } else {
        Write-Host "[SYSTEM] Deactivate command not found; continue without explicit deactivation." -ForegroundColor Yellow
    }
}
