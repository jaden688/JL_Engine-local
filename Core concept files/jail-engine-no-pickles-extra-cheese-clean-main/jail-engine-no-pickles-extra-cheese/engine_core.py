...
class JLEngineCore:
    ...
    def __init__(self, config: EngineConfig | None = None) -> None:
        ...
        self.last_scene_reaction: Dict[str, Any] = {}
    ...
    def set_persona(self, persona_name: str) -> None:
        ...
        if persona_path and os.path.exists(persona_path):
            try:
                self.current_persona_data = dict(load_mpf(persona_path))
            except Exception as exc:
    ...
    def smoke_test_engine(self, user_message: str) -> Tuple[str, Dict[str, Any]]:
        ...
        try:
            ...
            reply_text, meta = backends.ensure_text_and_metadata(result, backend)
        except Exception as exc:
    ...
    def process_turn(self, user_text: str,
        ...
        scene_reaction = self._assess_scene_impact(user_text or "")
        self.last_scene_reaction = scene_reaction or {}
        ...
        if scene_reaction:
            try:
                scene_bias = float(scene_reaction.get("aperture_bias", 0.0) or 0.0)
        ...
        self.emotional_aperture.update_from_signals(
            ...
            aperture_bias=advisory_payload.get("emotional_drift", 0.0) + scene_bias,
        )
        if scene_reaction:
            intensity = float(scene_reaction.get("intensity", 0.0) or 0.0)
            if intensity >= 0.2:
                emotion = scene_reaction.get("emotion") if intensity >= 0.35 else None
                self.emotional_aperture.update_from_signal(
                    emotion=emotion,
                    focus_delta=float(scene_reaction.get("focus_delta", 0.0) or 0.0),
                    overload_delta=float(scene_reaction.get("overload_delta", 0.0) or 0.0),
                )
        aperture_state = self.emotional_aperture.get_state()
        if isinstance(self.persona_state, dict):
        ...
        # 8) Task intent & Dynamic Context Injection
        ...
        if self.persona_manager:
            try:
                ...
                # Apply Dynamic Context Injection by passing intent for persona trait filtering
                persona_projection = self.persona_manager.get_projection(intent=task_intent)
            except Exception as exc:
        ...
        persona_output_raw = backend.generate(messages, options=options)
        persona_output, backend_meta = backends.ensure_text_and_metadata(persona_output_raw, backend)
        ...
        telemetry = {
            ...
            "behavior_blend": behavior_blend,
            "aperture_state": aperture_state,
            "scene_reaction": scene_reaction or {},
            "cognitive_mode": dominant_mode,
            "drift": {
    ...
    def _tqa_advisory_from_snapshot(self, snapshot: Dict[str, Any]) -> List[str]:
        if not snapshot:
            return []
        
        # Access future metrics from the dict-based snapshot
        ...
        stability_index = snapshot.get("stability_index", 1.0)
        if stability_index < 0.4:
            suggestions.append("CRITICAL: System stability compromised. Collapse imminent.")
    ...
    def _run_tqa_advisory(
        ...
        # 1. Update present state with current turn data
        ...
        # 2. Update future projection based on resolved context
        self._update_temporal_projection(risk_level, cognitive_load, overload_level)
        
        # 3. Check for emergency stability collapse BEFORE the turn ends
        snapshot_pre = self.tqa_layer.snapshot()
        if snapshot_pre.get("stability_index", 1.0) < 0.25:
            logger.warning("[TQA] Emergency stability collapse triggered: Index < 0.25")
            self.tqa_layer.collapse(
                reason="emergency_stability_limit",
                outcome="partial_success_controlled_fail",
                error_signature="STABILITY_BREACH"
            )
        else:
            # 4. Normal turn-complete collapse
            error_signature = context.get("error_signature")
        ...
        # 5. Generate final advisory from the post-collapse state
        snapshot = self.tqa_layer.snapshot()
        advisory = self._tqa_advisory_from_snapshot(snapshot)
        
        logger.info(
            "[TQA] Advisory phase=%s strength=%.2f suggestions=%d stability=%.4f",
            phase,
            self.tqa_strength,
            len(advisory),
            snapshot.get("stability_index", 1.0)
        )
    ...
    def _run_feedback_reflection(
        ...
        try:
            ...
            analysis_raw = backend.generate([{"role": "system", "content": prompt}], options=opts)
            analysis, _meta = backends.ensure_text_and_metadata(analysis_raw, backend)
    ...
    def _assess_scene_impact(self, text: str) -> Dict[str, Any]:
        if not text or not isinstance(text, str):
            return {}
        lowered = text.lower()
        tokens = set(re.findall(r"[a-z']+", lowered))

        def score_hits(words: list[str], phrases: list[str], boost_words: list[str]) -> int:
            hits = sum(1 for word in words if word in tokens)
            hits += sum(2 for phrase in phrases if phrase in lowered)
            hits += sum(2 for word in boost_words if word in tokens)
            return hits
        ...
        categories = [
            {
                "id": "injury",
                "emotion": "pain",
                "prompt": "Acknowledge injuries or physical harm with a brief, human reaction (shock/pain), without gore.",
                "words": [
                    "injured", "wounded", "hurt", "pain", "bleeding", "bleed", "stabbed", "shot", "burned",
                    "broken", "fractured", "torn", "ripped", "maimed",
                ],
                "phrases": [
                    "cut off", "severed hand", "severed arm", "lost a hand", "lost his hand", "lost her hand",
                    "lost their hand", "ripped off", "tore off",
                ],
                "boost": ["severed", "amputated", "decapitated", "mutilated"],
                "scale": 3.0,
                "focus": -0.12,
                "overload": 0.48,
                "aperture_bias": 0.18,
            },
            {
                "id": "threat",
                "emotion": "fear",
                "prompt": "Show alertness or fear when danger or threats appear. Do not ignore the risk.",
                "words": [
                    "threat", "threatened", "danger", "risky", "ambush", "attacked", "attack", "chasing",
                    "hostile", "enemy", "monster", "terror", "panic", "scared", "afraid", "terrified",
                    "trap", "explosion",
                ],
                "phrases": ["in danger", "under attack", "about to die", "coming for you"],
                "boost": ["ambush", "assassinate", "explosion"],
                "scale": 2.5,
                "focus": -0.08,
                "overload": 0.38,
                "aperture_bias": 0.14,
            },
            {
                "id": "anger",
                "emotion": "anger",
                "prompt": "Let anger show through when provoked, but keep it controlled and in character.",
                "words": [
                    "angry", "furious", "rage", "raging", "mad", "pissed", "enraged", "livid", "irate", "seething",
                    "betrayed", "backstabbed", "insulted", "humiliated",
                ],
                "phrases": ["saw red", "lost their temper", "lost his temper", "lost her temper"],
                "boost": ["furious", "enraged", "betrayed"],
                "scale": 2.5,
                "focus": 0.12,
                "overload": 0.24,
                "aperture_bias": 0.08,
            },
            {
                "id": "jealousy",
                "emotion": "jealousy",
                "prompt": "Show jealousy or envy subtly when replaced, excluded, or compared.",
                "words": ["jealous", "jealousy", "envy", "envious", "resentful", "ignored", "replaced"],
                "phrases": ["left out", "chosen over", "picked over", "second choice"],
                "boost": ["jealousy", "envious"],
                "scale": 2.5,
                "focus": 0.08,
                "overload": 0.2,
                "aperture_bias": 0.06,
            },
            {
                "id": "joy",
                "emotion": "joy",
                "prompt": "Express joy or relief openly when something good happens.",
                "words": [
                    "joy", "joyful", "happy", "delighted", "excited", "thrilled", "ecstatic", "smiling", "laughing",
                    "laugh", "celebrate", "celebration", "cheer", "cheered", "win", "won", "victory", "hug",
                    "kiss", "love", "proud",
                ],
                "phrases": ["over the moon", "on cloud nine", "couldn't stop smiling"],
                "boost": ["ecstatic", "thrilled", "victory"],
                "scale": 3.0,
                "focus": 0.12,
                "overload": -0.28,
                "aperture_bias": 0.1,
            },
            {
                "id": "sadness",
                "emotion": "sadness",
                "prompt": "Acknowledge loss or grief with softer tone and visible sadness.",
                "words": [
                    "sad", "sorrow", "grief", "mourning", "heartbroken", "cry", "crying", "tears", "devastated",
                    "loss", "died", "death", "funeral",
                ],
                "phrases": ["fell apart", "lost someone", "couldn't breathe"],
                "boost": ["devastated", "heartbroken"],
                "scale": 2.8,
                "focus": -0.12,
                "overload": 0.18,
                "aperture_bias": 0.08,
        ...
        for category in categories:
            hits = score_hits(category["words"], category["phrases"], category["boost"])
            if hits <= 0:
                continue
            intensity = min(1.0, hits / float(category["scale"]))
            if not best or intensity > best["intensity"]:
                best = {
                    "category": category["id"],
                    "emotion": category["emotion"],
                    "intensity": intensity,
                    "focus_delta": category["focus"] * intensity,
                    "overload_delta": category["overload"] * intensity,
                    "aperture_bias": category["aperture_bias"] * intensity,
    ...
    def _build_messages(
        ...
        scene_reaction: Dict[str, Any] | None = None,
    ) -> list[dict]:
        """
        ...
        if scene_reaction:
            try:
                intensity = round(float(scene_reaction.get("intensity", 0.0) or 0.0), 2)
            except (TypeError, ValueError):
                intensity = 0.0
            category = scene_reaction.get("category") or "unknown"
            system_chunks.append("\nSCENE AWARENESS:")
            system_chunks.append(f"- Detected: {category} (intensity {intensity})")
            prompt = scene_reaction.get("prompt")
            if prompt:
                system_chunks.append(f"- {prompt}")
